...
brewing.engine = {}
brewing.engine.players = {}
brewing.engine.effects = {}

brewing.engine.effects.phys_override = function(sname, name, fname, time, sdata, flags)
	local def = {
...
brewing.engine.effects.fixhp = function(sname, name, fname, time, sdata, flags)
	local def = {
		on_use = function(itemstack, user, pointed_thing)
			brewing.magic_sound("to_player", user, "brewing_magic_sound")
			brewing.magic_aura(user, user:get_pos(), "player", "default")
...
brewing.engine.effects.air = function(sname, name, fname, time, sdata, flags)
	local def = {
		on_use = function(itemstack, user, pointed_thing)
			brewing.magic_sound("to_player", user, "brewing_magic_sound")
			brewing.magic_aura(user, user:get_pos(), "player", "default")
...
brewing.engine.effects.blowup = function(sname, name, fname, time, sdata, flags)
	local def = {
		on_use = function(itemstack, user, pointed_thing)
			brewing.magic_sound("to_player", user, "brewing_magic_sound")
			brewing.magic_aura(user, user:get_pos(), "player", "default")
...
brewing.engine.effects.set_invisibility = function(player) -- hide player and name tag	
	prop = {
		visual_size = {x = 0, y = 0},	
	}
...
brewing.engine.effects.set_visibility = function(player) -- show player and tag			
	prop = {
		visual_size = {x = 1, y = 1},	
	}
	player:set_nametag_attributes({
...
brewing.engine.effects.freeze = function(pointed_thing, user, emitter)			
	...
	if not(is_freeze_entity) then --do not freeze over a already freeze entity
		...
		minetest.after(brewing.settings.freeze_time, function(freeze_entity, entity_to_freeze, pos) --Unfreeze after x seconds
		...
		end, freeze_entity, entity_to_freeze, pos)
...
brewing.engine.effects.aqua = function(pointed_thing, user, emitter)
...
brewing.engine.effects.ttl = 1
brewing.engine.effects.revertsky = function()
...
brewing.engine.effects.light_strike = function(user, enemy)
...
brewing.engine.register_potion = function(sname, name, fname, time, def)
	local tps = {"add", "sub"}
	for t=1, #tps do
...
brewing.engine.effects.register_liquid = function(name, hname, funct)
