...
brewing = {
	...
	settings = {
		...
		-- Cork
		cork = "brewing:cork",
		cork_tool = "brewing:little_razor",
		cork_count = 16,
		-- end cork
		donut_eat_hp = 7,
		lily_hp = 30,
	...
	get_cork = function(pos, node, player)
		local result = false
		local wielded_item = player:get_wielded_item()
		if wielded_item:get_name() == brewing.settings.cork_tool then
			local inv = player:get_inventory()
			if inv:room_for_item("main", ItemStack(brewing.settings.cork)) then
				inv:add_item("main", ItemStack(brewing.settings.cork.." "..tostring(brewing.settings.cork_count))) --add cork to player's inventory
				minetest.set_node(pos, {name=node.name.."_nobark"}) -- replace the trunk
				brewing.magic_sound("to_player", player, "brewing_change")
				result = true
			end
		end
		return result
